...
public class logtime {
    ...
    public static class LogReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
        ...
        private Map<Text, IntWritable> logTimes = new HashMap<>();
        ...
        public void reduce(Text key, Iterable<IntWritable> values, Context context)
            ...
            logTimes.put(new Text(key), new IntWritable(totalTime));
        }

        @Override
        protected void cleanup(Context context) throws IOException, InterruptedException {
            logTimes.entrySet().stream()
                    .sorted(Map.Entry.<Text, IntWritable>comparingByValue(
                            Comparator.comparingInt(IntWritable::get)).reversed())
                    .forEach(entry -> {
                        try {
